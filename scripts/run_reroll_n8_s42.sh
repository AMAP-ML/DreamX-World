...
for gpu in $(seq 0 $((NS-1))); do
  CUDA_VISIBLE_DEVICES=$gpu $PY demo_closed_loop.py \
    ...
    --data_path configs/dreamx/eval.json --output_folder demo_reroll_n8_s42/ \
    --num_output_frames 123 --seed 42 --chunk_relative --modes open reroll \
    --reroll_n 7 --reroll_thresh 0.2 \
    --drift_stride 2 --shard_id $gpu --num_shards $NS > /tmp/reroll_n8_s42_$gpu.log 2>&1 &
  pids+=($!)
done
fail=0; for i in "${!pids[@]}"; do wait "${pids[$i]}" && echo "shard $i ok" || fail=1; done
echo "ALL_n8_s42_DONE fail=$fail" > /home/ma-user/work/dataset/VidGen_data_obs/DreamX-World/.n8_s42_done
