...
for gpu in $(seq 0 $((NSHARDS-1))); do
  CUDA_VISIBLE_DEVICES=${gpu} "$DREAMX_PY" demo_closed_loop.py \
    ...
    --output_folder demo_goldctx_amp_30s/ \
    --num_output_frames 123 --seed 42 --chunk_relative \
    --modes open gold_ctx \
    --kp2 0.05 --ki2 0.12 --leak2 0.9 --deadband2 0.10 --gain_max 0.5 \
    --decode_every 3 --gold_thresh 0.15 --ctx_base 0.1 --ctx_k 3.0 --ctx_max 0.5 --ctx_deadband 0.1 --drift_stride 2 \
    --shard_id ${gpu} --num_shards ${NSHARDS} > /tmp/goldctxamp_${gpu}.log 2>&1 &
  pids+=($!)
done
echo "launched ${#pids[@]} shards"
fail=0
for i in "${!pids[@]}"; do wait "${pids[$i]}" && echo "shard ${i} exit=0" || { echo "shard ${i} FAIL"; fail=1; }; done
echo "ALL_GCAMP_DONE fail=${fail}"
