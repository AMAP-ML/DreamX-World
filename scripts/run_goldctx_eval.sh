...
for gpu in $(seq 0 $((NSHARDS-1))); do
  ( CUDA_VISIBLE_DEVICES=${gpu} "$DREAMX_PY" window_eval.py --folder demo_goldctx_30s/ \
      --modes open gold_ctx --stride 2 --lags 4 8 --shard_id ${gpu} --num_shards ${NSHARDS}
    CUDA_VISIBLE_DEVICES="" "$DREAMX_PY" camera_track.py --folder demo_goldctx_30s/ \
      --modes open gold_ctx --stride 8 --shard_id ${gpu} --num_shards ${NSHARDS} \
