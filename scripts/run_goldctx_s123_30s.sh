...
for gpu in $(seq 0 $((NSHARDS-1))); do
  CUDA_VISIBLE_DEVICES=${gpu} "$DREAMX_PY" demo_closed_loop.py \
    ...
    --output_folder demo_goldctx_s123_30s/ \
    --num_output_frames 123 --seed 123 --chunk_relative \
    --modes open gold_ctx \
