...
for gpu in $(seq 0 7); do
  CUDA_VISIBLE_DEVICES=$gpu $PY demo_closed_loop.py \
    ...
    --data_path configs/dreamx/eval.json --output_folder demo_reroll_s123_30s/ \
    --num_output_frames 123 --seed 123 --chunk_relative --modes open reroll \
    --reroll_n 2 --reroll_thresh 0.2 \
    --drift_stride 2 --shard_id $gpu --num_shards $NS > /tmp/reroll_s123_$gpu.log 2>&1 &
  pids+=($!)
done
fail=0; for i in "${!pids[@]}"; do wait "${pids[$i]}" && echo "shard $i ok" || fail=1; done
echo "ALL_REROLL_s123_DONE fail=$fail"
